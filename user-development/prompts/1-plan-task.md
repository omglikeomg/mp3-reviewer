...
1. **Read the source code directly** — the source code is the source of truth. Read the relevant Go files for the packages and areas you'll be planning changes to. Use `diagrams/FOLDER-STRUCTURE.md` for quick orientation on the project layout if needed. Do not rely on `.mmd` diagram files for implementation understanding — they are visual documentation for human readers and may lag behind the code.
...
7. **Include a diagram update section** — the plan must include a section or checklist item specifying which diagrams in `diagrams/` need to be updated as part of this task's documentation deliverables, and what changes are expected (new types, new states, new dependencies, etc.). Diagrams are maintained for human readers, not as an agent source of truth.
