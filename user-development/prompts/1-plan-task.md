...
1. **Read the source code directly** — the source code is the source of truth. Read the relevant Go files for the packages and areas you'll be planning changes to. Use `agent-development/agent-specs/FOLDER-STRUCTURE.md` for quick orientation on the project layout if needed.
...
7. **Include a spec update note** — if the task introduces new packages, interfaces, or structural changes, the plan must include a checklist item to update `agent-development/agent-specs/architecture-breakdown.md` and/or `agent-development/agent-specs/FOLDER-STRUCTURE.md` accordingly.
